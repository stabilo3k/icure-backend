...
map = function (doc) {
  if(doc.entityType === 'org.taktik.icure.entities.embed.Medication' && doc.subType && doc.descr && doc.entity[0].compoundPrescription) {
    var descr_compound = (doc.descr + ' ' + doc.entity[0].compoundPrescription);
    descr_compound = descr_compound.toLowerCase().replace(/[^A-Za-z ]/g,function(a){return latin_map[a]||""});
    var words = descr_compound.split(' ');
    ...
    for(var i = 0; i < words.length; i++) {
      if(words[i].length > 2){
        emit([doc.subType, words[i]], summary)
      }
    }
  }
}